...
#!/usr/bin/env bash
set -e
if ! id powergateway >/dev/null 2>&1; then useradd --system --home /var/lib/powergateway --shell /usr/sbin/nologin powergateway; fi
...
if [[ ${1:-} == purge ]]; then rm -rf /var/lib/powergateway; userdel powergateway >/dev/null 2>&1 || true; fi
EOF
