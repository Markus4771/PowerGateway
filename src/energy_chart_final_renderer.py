...
SCRIPT = r'''
(function(){
  let viewStart=null,viewEnd=null,drag=null,bound=false,lastDatasetKey='';
  ...
  function datasetKey(){const all=allPoints();if(!all.length)return (energyData&&energyData.range)||'empty';return [(energyData&&energyData.range)||'',Number(all[0].ts),Number(all[all.length-1].ts),all.length].join(':')}
  function ensureDataset(){const key=datasetKey();if(lastDatasetKey&&key!==lastDatasetKey){viewStart=null;viewEnd=null;drag=null}lastDatasetKey=key}
  ...
  window.energyZoomBy=function(factor,center=null){ensureDataset();const all=allPoints();if(all.length<2)return;const [fs,fe]=fullBounds();const s=viewStart??fs,e=viewEnd??fe,mid=center??((s+e)/2);const span=Math.min(fe-fs,Math.max(10,(e-s)*factor));[viewStart,viewEnd]=clamp(mid-span/2,mid+span/2);window.renderEnergyChart()};
  window.energyResetZoom=function(){viewStart=null;viewEnd=null;drag=null;window.renderEnergyChart()};
  function bind(){if(bound)return;const svg=document.getElementById('energyChart');if(!svg)return;bound=true;svg.style.touchAction='none';svg.addEventListener('wheel',function(ev){ev.preventDefault();ensureDataset();const pts=visiblePoints();if(pts.length<2)return;const r=svg.getBoundingClientRect();const ratio=Math.max(0,Math.min(1,(ev.clientX-r.left)/Math.max(1,r.width)));const center=Number(pts[0].ts)+ratio*(Number(pts[pts.length-1].ts)-Number(pts[0].ts));window.energyZoomBy(ev.deltaY<0?0.65:1.5,center)},{passive:false});svg.addEventListener('dblclick',function(){window.energyResetZoom()});svg.addEventListener('pointerdown',function(ev){ensureDataset();const pts=visiblePoints();if(pts.length<2)return;svg.setPointerCapture(ev.pointerId);drag={x:ev.clientX,start:viewStart??Number(pts[0].ts),end:viewEnd??Number(pts[pts.length-1].ts)}});svg.addEventListener('pointermove',function(ev){if(!drag)return;const r=svg.getBoundingClientRect();const delta=(ev.clientX-drag.x)/Math.max(1,r.width)*(drag.end-drag.start);[viewStart,viewEnd]=clamp(drag.start-delta,drag.end-delta);window.renderEnergyChart()});const stop=function(){drag=null};svg.addEventListener('pointerup',stop);svg.addEventListener('pointercancel',stop)}
