...
def _settings() -> dict[str, Any]:
    result = {'price_eur_kwh': 0.31, 'default_range': '7d'}
...
def history(name: str) -> dict[str, Any]:
    ...
    with _connect() as db:
        rows = db.execute('''SELECT (ts / ?) * ? bucket,
            ...
            GROUP BY bucket ORDER BY bucket''', (bucket, bucket, start, end)).fetchall()
        first_last = db.execute('''SELECT
            ...
            (SELECT power_w FROM measurements WHERE ts BETWEEN ? AND ? AND power_w IS NOT NULL ORDER BY ts DESC LIMIT 1) current_power''', (start, end, start, end, start, end)).fetchone()
    points = []
    estimated_kwh = 0.0
    previous_ts = None
    previous_power = None
    ...
    price = float(_settings().get('price_eur_kwh', 0.31) or 0)
    return {'ok': True, 'range': name, 'start': start, 'end': end, 'bucket_seconds': bucket, 'points': points, 'summary': {'current_power_w': first_last['current_power'] if first_last else None, 'consumption_kwh': round(consumption, 3), 'cost_eur': round(consumption * price, 2), 'samples': len(points)}, 'settings': _settings()}
...
async function loadEnergyHistory(){try{const range=$('energyRange').value||'7d';const d=await api('/_internal/energy/history?range='+encodeURIComponent(range));energyData=d;const s=d.summary||{};$('energyPower').textContent=energyFmt(s.current_power_w)+' W';$('energyConsumption').textContent=energyFmt(s.consumption_kwh,3)+' kWh';$('energyCost').textContent=energyFmt(s.cost_eur,2)+' €';$('energySamples').textContent=String(s.samples||0);$('energyPrice').value=(d.settings||{}).price_eur_kwh??0.31;$('energyPeriod').textContent=new Date(d.start*1000).toLocaleString('de-DE')+' – '+new Date(d.end*1000).toLocaleString('de-DE');renderEnergyChart()}catch(e){notice(e.message,false)}}
async function saveEnergySettings(){try{const d=await api('/_internal/energy/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({price_eur_kwh:Number($('energyPrice').value),default_range:'7d'})});notice('Strompreis gespeichert.',true);await loadEnergyHistory()}catch(e){notice(e.message,false)}}
'''
