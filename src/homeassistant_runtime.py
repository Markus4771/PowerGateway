...
from noip_client import load_config as load_noip, load_status as load_noip_status
...
def connector_get() -> Response:
    return jsonify({'config': public_ha(load_ha()), 'noip': public_noip(load_noip()), 'noip_status': load_noip_status()})
...
def noip_save() -> Response:
    try:
        ...
        return jsonify({'error': str(exc)}), 400
    return jsonify({'ok': True, 'config': public_noip(config), 'status': load_noip_status(), 'message': 'No-IP-Einstellungen gespeichert.'})
...
def noip_status() -> Response:
    return jsonify({'config': public_noip(load_noip()), 'status': load_noip_status()})
...
function noipPayload(){return {enabled:$('noipEnabled').checked,hostname:$('noipHostname').value.trim(),username:$('noipUsername').value.trim(),password:$('noipPassword').value,interval_minutes:Number($('noipInterval').value),restart_ssh_on_ip_change:$('noipRestartSsh').checked}}
async function saveNoip(){try{const d=await api('/_internal/noip',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(noipPayload())});notice(d.message);val('noipPassword','');showNoipStatus(d.status||{})}catch(e){notice(e.message,false)}}
...
async function diagnoseNoip(){try{const d=await api('/_internal/noip/diagnostics');$('noipResult').textContent=JSON.stringify(d,null,2)}catch(e){notice(e.message,false)}}
'''
