...
from noip_client import load_config as load_noip, public_config as public_noip, save_config as save_noip, update as update_noip
...
def connector_get() -> Response:
    return jsonify({'config': public_ha(load_ha()), 'noip': public_noip(load_noip())})
...
def noip_save() -> Response:
    try:
        ...
        return jsonify({'error': str(exc)}), 400
    return jsonify({'ok': True, 'config': public_noip(config), 'message': 'No-IP-Einstellungen gespeichert.'})
...
async function saveNoip(){try{const d=await api('/_internal/noip',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(noipPayload())});notice(d.message);val('noipPassword','')}catch(e){notice(e.message,false)}}
async function testNoip(){try{const r=await fetch('/_internal/noip/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(noipPayload())}),d=await r.json();$('noipResult').textContent=JSON.stringify(d,null,2);if(!r.ok)throw new Error(d.error||d.message);notice(d.message)}catch(e){notice(e.message,false)}}
'''
