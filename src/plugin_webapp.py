...
def modules_status()->Response:
    ...
    result['optional_energy_modules']=energy_optional_loader.LOAD_RESULTS
    return jsonify(result)
@app.get('/_internal/modules/diagnostics')
@legacy.login_required
def modules_diagnostics()->Response:
    result=manager.diagnostics(); result['events']=module_status()['events']; result['optional_energy_modules']=energy_optional_loader.LOAD_RESULTS; return jsonify(result)
page=runtime.PAGE
